...
for (const mode of ['inline', 'tool'] as const) {
  test(`sets a standing goal through ${mode} and updates the live indicator`, async () => {
    ...
    try {
      ...
      const title = mode === 'inline' ? 'Verify inline standing goals' : 'Verify the agent-created standing goal'
      if (mode === 'inline') {
        await composer.fill('Use the current project. /go')
