...
class AIAgent:
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        # explicit ``background=false`` is the join path for work whose result
        # is required before this turn can finish (for example, a final review).
        # Delegations from an ORCHESTRATOR SUBAGENT always stay synchronous: the
        # orchestrator must consume its workers' results inside its own turn.
        _is_subagent = getattr(self, "_delegate_depth", 0) > 0
        _requested_background = function_args.get("background")
        _background = (
            False
            if _is_subagent
            else (
                is_truthy_value(_requested_background, default=True)
                if _requested_background is not None
                else True
            )
        )
