...
export const INSPECTION_KEYS = ['selector', 'limit', 'cursor', 'maxChars', 'fields', 'visibleOnly']
...
export function validControlArguments(method: string, a: Record<string, unknown>, wire = false): boolean {
  ...
  if (method === 'click') { return keys(['target', 'button']) && target() && (a.button === undefined || ['left', 'right', 'middle'].includes(String(a.button))) }

  if (method === 'hover') { return keys(['target']) && target() }

  if (method === 'type') { return keys(['target', 'text', 'submit']) && target() && typeof a.text === 'string' && a.text.length <= 100_000 && (a.submit === undefined || typeof a.submit === 'boolean') }
  ...
  if (method === 'scroll') { return keys(['target', 'deltaX', 'deltaY']) && (a.target === undefined || target()) && [a.deltaX ?? 0, a.deltaY ?? 0].every(v => typeof v === 'number' && Number.isFinite(v) && Math.abs(v) <= 100_000) && (Number(a.deltaX ?? 0) !== 0 || Number(a.deltaY ?? 0) !== 0) }
  ...
  if (a.action === 'drag') { return controlKeys(['target', 'destination']) && target() && text(a.destination) }
