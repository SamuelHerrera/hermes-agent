...
- `stop --dir PATH --force`: destructive; kills live PTY leaders (SIGKILL on POSIX; ConPTY kill on Windows), closes admission, acknowledges shutdown, then removes discovery/lock and exits. The acknowledgment precedes final process exit by a short cleanup grace.
...
Each attach increments a writer generation. Input, resize and detach reject stale generations (`STALE_WRITER`); read is non-owning. HTTP disconnect has no mutating effect. A new attach takes over immediately; a delayed old detach cannot revoke it. Lease-changing requests are serialized around asynchronous screen barriers. There is no writer timeout to mistake an idle connection for a dead process.
...
Snapshot includes `{cols,rows,seq,data,initial,exact,replay}`:

- `data`: serializer's ANSI reconstruction of normal and alternate visible buffers plus retained scrollback. Useful for visual inspection, **not sufficient for arbitrary differential continuation**.
- `replay`: from-start decoded output and resize events, bounded to **2 MiB including event metadata**. Preserves partial ANSI parser state, scroll regions, origin modes, resize history, etc. while available.
- `exact`: false after that replay budget is exceeded, permanently for that session. `replay:null`. Session continues running and attached clients still receive deltas, but arbitrary reattachment fidelity is now an explicit release gate.

`import { restoreScreen } from '@hermes/terminal-host/screen'` resets a terminal, restores original dimensions, and replays each data/resize event sequentially. It **throws `RECONSTRUCTION_LIMIT` for exact:false**, rather than pretending serializer output restores unsupported modes. Use the same xterm options (scrollback 2000, default Unicode width implementation) and no incompatible parser addons. Title/graphics/custom OSC integrations are not validated.

**Host alone answers device queries.** Renderer-generated terminal replies during both restoration AND live playback must never be sent to input. Adapters must separately route physical keyboard/paste, not blindly forward renderer `onData` replies. The helper cannot control listeners installed by a caller. No synthetic Ctrl-L, shell commands or prior user input are replayed to the PTY.

Before unrestricted persistent-TUI release, implement and verify a bounded checkpoint that preserves all required parser/mode state beyond the exact replay cap. The current implementation does not claim that addon serialization solves this.
...
macOS executed. Native Linux and Windows (including ConPTY, DACL inheritance, detached lifetime under launcher job objects, npm executable behavior and native helper staging) remain **unexecuted release gates**. No Desktop/backend restart or renderer visual acceptance is claimed by these standalone tests.
